...
// DB에서 linkareer 컬렉션의 모든 타이틀을 배열로 가져오는 함수
async function loadLastPostTitles() {
  const posts = await LinkareerPost.find({}, 'title'); // 모든 포스트의 title 필드를 가져옴
  lastPostTitles = posts.map((post) => post.title); // title 필드만 추출
}

async function getLinkareerPosts() {
  // DB에서 마지막 포스트들을 불러옴
  await loadLastPostTitles();

  console.log('lastPostTitles:', lastPostTitles);

  const { browser, newPostTitles, titlesToDelete } = await linkareerScrape(
    lastPostTitles
  ); // Linkareer 스크래핑 후 새로운 포스트와 삭제할 포스트의 제목을 가져옴

  // 삭제할 포스트가 있으면 DB에서 삭제
  if (titlesToDelete.length > 0) {
    await LinkareerPost.deleteMany({ title: { $in: titlesToDelete } });
  ...
  if (newPostTitles.length > 0) {
    for (const title of newPostTitles) {
      ...
      const newPost = new LinkareerPost({
        title: joinedTitle,
        link: joinedLink,
      });
      await newPost.save();
...
module.exports = getLinkareerPosts;
